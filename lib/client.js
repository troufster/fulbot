...
class Client extends EventEmitter {
  ...
  action(data) {
    let to = data.originalmessage.to;
    if(!this.canSpeak(to)) return;

    switch(data.action) {
      case "kick":
        //Original message should be to chan + client should be op
        if(Utils.isChanMessage(to) && this.isOperator(to, this.irc.nick)){
          this.irc.send(data.action,data.from,data.to, data.reason);
        }
        break;
    }
  }
